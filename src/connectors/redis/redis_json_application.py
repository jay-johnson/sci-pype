...
class RedisJSONApplication(BaseRedisApplication):
    ...
    def disconnect(self):
        if self.m_debug:
            self.lg("PubR Disconnecting from Redis(" + str(self.m_host_address) + ":" + str(self.m_port) + ") with JSON queue(" + str(self.m_queue_name) + ")", 7)
    ...
    def connect(self):
        if self.m_debug:
            if self.m_redis_password:
                self.lg("Connecting to Secure Redis(" + str(self.m_host_address) + ":" + str(self.m_port) + ") DB(" + str(self.m_db) + ") with JSON queue(" + str(self.m_queue_name) + ")", 7)
            else:
                self.lg("Connecting to Unsecure Redis(" + str(self.m_host_address) + ":" + str(self.m_port) + ") DB(" + str(self.m_db) + ") with JSON queue(" + str(self.m_queue_name) + ")", 7)
        # end of debugging

        self.m_rw = RedisWrapper(self.m_queue_name, serializer=json, host=self.m_host_address, port=int(self.m_port), db=int(self.m_db), password=self.m_redis_password)
        return None
    # end of connect
    ...
    def get_single_cache(self, key=None):
        ...
        try:
            ...
            self.lg("ERROR: Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + key + ") Received a Non-JSON Formatted Message Ex(" + str(e) +")", 0)
        except Exception, e:
            test_for_redis_connectivity = str(e)
            if "Connection refused." in test_for_redis_connectivity:
                self.lg("ERROR: Connection Failed to Redis Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + key + ") JSON Encountered Ex(" + str(e) +") sleeping(" + str(self.m_sleep_for_connection_outage) + ")", 0)
                sleep(self.m_sleep_for_connection_outage)
            else:
                self.lg("Possible Error but the cache may not exist: Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + key + ") JSON Encountered Ex(" + str(e) +")", 7)
    ...
    def get_cached_set(self, start_idx=0, end_idx=-1, queue=None):
        ...
        try:
            ...
            self.lg("ERROR: Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + queue + ") Received a Non-JSON Formatted Message Ex(" + str(e) +")", 0)
        except Exception, e:
            test_for_redis_connectivity = str(e)
            if "Connection refused." in test_for_redis_connectivity:
                self.lg("ERROR: Connection Failed to Redis Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + queue + ") JSON Encountered Ex(" + str(e) +") sleeping(" + str(self.m_sleep_for_connection_outage) + ")", 0)
                sleep(self.m_sleep_for_connection_outage)
            else:
                self.lg("ERROR: Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + queue + ") JSON Encountered Ex(" + str(e) +")", 0)
    ...
    def get_message(self, block=False, key=None):
        ...
        try:
            ...
            self.lg("ERROR: Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + self.m_queue_name + ") Received a Non-JSON Formatted Message Ex(" + str(e) +")", 0)
        except Exception, e:
            test_for_redis_connectivity = str(e)
            if "Connection refused." in test_for_redis_connectivity:
                self.lg("ERROR: Connection Failed to Redis Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + self.m_queue_name + ") JSON Encountered Ex(" + str(e) +") sleeping(" + str(self.m_sleep_for_connection_outage) + ")")
                sleep(self.m_sleep_for_connection_outage)
            else:
                self.lg("ERROR: Address(" + self.m_host_address + ":" + str(self.m_port) + ") Queue(" + self.m_queue_name + ") JSON Encountered Ex(" + str(e) +")", 0)
    ...
    def wait_for_message_on_key(self, num_seconds, key):
        ...
        try:
            ...
            self.lg("ERROR: Address(" + self.m_host_address + ":" + str(self.m_port) + ") Key(" + key + ") Received a Non-JSON Formatted Message Ex(" + str(e) +")", 0)
        except Exception, e:
            test_for_redis_connectivity = str(e)
            if "Connection refused." in test_for_redis_connectivity:
                self.lg("ERROR: Connection Failed to Redis Address(" + self.m_host_address + ":" + str(self.m_port) + ") Key(" + key + ") JSON Encountered Ex(" + str(e) +") sleeping(" + str(self.m_sleep_for_connection_outage) + ")")
            else:
                self.lg("ERROR: Address(" + self.m_host_address + ":" + str(self.m_port) + ") Key(" + key + ") JSON Encountered Ex(" + str(e) +")", 0)
