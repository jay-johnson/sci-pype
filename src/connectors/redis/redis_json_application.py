...
class RedisJSONApplication(BaseRedisApplication):
    ...
    def connect(self):
        if self.m_debug:
            self.lg("PubR Connecting to Redis(" + str(self.m_host_address) + ":" + str(self.m_port) + ") with JSON queue(" + str(self.m_queue_name) + ")", 7)
        self.m_rw = RedisWrapper(self.m_queue_name, serializer=json, host=self.m_host_address, port=int(self.m_port), db=0)
        return None
